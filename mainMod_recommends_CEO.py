...
##            args.username = d['username']
##            args.password = d['password']
            args.username = 'arvind.baranwal@gmail.com'
            args.password = 'syntax123'
...
def scrape(field, review, author):
    ...
    def _scrape_subrating(i):
        try:
            ...
            tag = this_one.find_element_by_class_name('minor').get_attribute("innerHTML")
    ...
    def scrape_work_life_balance(review):
        for i in range(0,4):
            if _scrape_subrating(i)[0] == 'Work/Life Balance':
                return _scrape_subrating(i)[1]  
        return np.nan

    def scrape_culture_and_values(review):
        for i in range(0,4):
            if _scrape_subrating(i)[0] == 'Culture &amp; Values':
                return _scrape_subrating(i)[1]        
        return np.nan

    def scrape_career_opportunities(review):
        for i in range(0,4):
            if _scrape_subrating(i)[0] == 'Career Opportunities':
                return _scrape_subrating(i)[1]       
        return np.nan

    def scrape_comp_and_benefits(review):
        for i in range(0,4):
            if _scrape_subrating(i)[0] == 'Comp &amp; Benefits':
                return _scrape_subrating(i)[1]        
        return np.nan

    def scrape_senior_management(review):
        for i in range(0,4):
            if _scrape_subrating(i)[0] == 'Senior Management':
                return _scrape_subrating(i)[1]        
        return np.nan
    ...
    def scrape_ceo(review):
        result = ''
        try:
            for token in review.find_element_by_class_name('flex-grid').text.split('\n'):
                if 'CEO' in token:
                    result = token
        except:
            result =np.nan
...
def main():
    ...
    sign_in()
    data = pd.read_excel('C:/Users/arvind.baranwal/Downloads/liberty dec renewals/Dec_renewal_glassdoorLinks_cleaned.xlsx')
    urls = data['Glassdoor_review_link'].astype(str)
#    urls=["https://www.glassdoor.com/Reviews/Sullivan-and-Cromwell-Reviews-E3395.htm"]

    no_of_records_done = 0
    for i in range(0,len(urls)):
        ...
        try:
            ...
            while more_pages() and\
                ...
                reviews_df = extract_from_page()
                res = res.append(reviews_df)
            res['company_name'] = args.url 
            print("records scraped: "+str(len(res)))
            full_res = full_res.append(res)
            no_of_records_done +=1
        #            if(no_of_records_done%10==0):
        #            full_res.to_csv(args.file, index=False, encoding='utf-8')
            if i%20==0:
...
#    continue
    ...
    logger.info(f'Writing {len(full_res)} reviews to file {args.file}')
    full_res.to_csv(args.file, index=False, encoding='utf-8')
